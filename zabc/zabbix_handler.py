...
class ZabbixHandler:
    def __init__(self, keystone_host,keystone_admin_port, nova_api_host,nova_api_port, zabbix_host, admin_user, zabbix_admin_pass,
                 template_name, zabbix_proxy_name, keystone_auth, vm_meters):

        self.keystone_host = keystone_host
        self.keystone_admin_port = keystone_admin_port

        self.nova_api_host = nova_api_host
        self.nova_api_port = nova_api_port
    ...
    def check_instances(self):
        ...
        for item in servers[u'servers']:
            ...
            if response['result'] is False:
                ...
                log.info("VM Instance %s has already existed ... " %(item['name']+"-"+item['id']))
