...
class MyStreamLoggingFormatter(logging.Formatter):
    simple_format = '[%(asctime)-15s: %(funcName)s] %(message)s'
    warn_format   = '\033[00;33m[%(asctime)-15s: %(filename)s:%(lineno)s:%(funcName)s: %(levelname)s]\033[00m %(message)s'
    error_format   = '\033[00;31m[%(asctime)-15s: %(filename)s:%(lineno)s:%(funcName)s: %(levelname)s]\033[00m %(message)s'
    FORMATS = {logging.DEBUG:   simple_format,
               logging.ERROR:   error_format,
               logging.WARNING: warn_format,
...
class MyFileLoggingFormatter(logging.Formatter):
...
class ZbcLogger(logging.getLoggerClass()):
